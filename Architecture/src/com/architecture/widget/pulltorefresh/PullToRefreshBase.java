 ...
 *******************************************************************************/
package com.architecture.widget.pulltorefresh;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public void addView(View child, int index, ViewGroup.LayoutParams params) {
		if (DEBUG) {
			LogcatUtils.d(LOG_TAG, "addView: " + child.getClass().getSimpleName());
		}
			...
			case MotionEvent.ACTION_UP: {
				if (mIsBeingDragged) {
					...
					if (mState == State.RELEASE_TO_REFRESH && 
						((null != mOnRefreshListener && mOnRefreshListener.canRefresh()) || 
						 (null != mOnRefreshListener2 && mOnRefreshListener2.canRefresh()))) {
	...
	public final void setMode(Mode mode) {
		if (mode != mMode) {
			if (DEBUG) {
				LogcatUtils.d(LOG_TAG, "Setting mode to: " + mode);
			}
			mMode = mode;
			updateUIForMode();
		}
	...
	final void setState(State state, final boolean... params) {
		mState = state;
		if (DEBUG) {
			LogcatUtils.d(LOG_TAG, "State: " + mState.name());
		}
	...
	protected final void onSizeChanged(int w, int h, int oldw, int oldh) {
		if (DEBUG) {
			LogcatUtils.d(LOG_TAG, String.format("onSizeChanged. W: %d, H: %d", w, h));
		}
	...
	protected final void refreshLoadingViewsSize() {
		...
		if (DEBUG) {
			LogcatUtils.d(LOG_TAG, String.format("Setting Padding. L: %d, T: %d, R: %d, B: %d", pLeft, pTop, pRight, pBottom));
		}
	...
	protected final void setHeaderScroll(int value) {
		if (DEBUG) {
			LogcatUtils.d(LOG_TAG, "setHeaderScroll: " + value);
		}
	...
	protected void init(Context context, AttributeSet attrs) {
		...
		ViewConfiguration config = ViewConfiguration.get(context.getApplicationContext());
	 ...
	 * @author Chris Banes
	 */
	public static abstract class OnRefreshListener<V extends View> {

		/**
		 * onRefresh will be called for both a Pull from start, and Pull from
		 * end
		 */
		public abstract void onRefresh(final PullToRefreshBase<V> refreshView);
