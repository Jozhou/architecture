package com.architecture.widget.stickylistheaders;
...
import com.architecture.widget.stickylistheaders.WrapperViewList.LifeCycleListener;
...
//		System.out.println("-----------<> " + position + "--" + isStartOfSection);
		if (!isStartOfSection) {
	...
	@SuppressLint("NewApi")
	@TargetApi(Build.VERSION_CODES.FROYO)
	public void smoothScrollToPositionWithHeadView(int position) {
		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
			mList.smoothScrollToPosition(position);
		} else {
			int offset = mAdapter == null ? 0 : getHeaderOverlap(position);
			offset -= mClippingToPadding ? 0 : mPaddingTop;
			offset -= getItemHeight()*mList.getHeaderViewsCount();
			mList.smoothScrollToPositionFromTop(position, offset);
		}
	}
	...
	public int getItemHeight(){
		...
		View itemView = mAdapter.getView(getHeaderViewsCount()+2, null, mList);
		ensureHeaderHasCorrectLayoutParams(itemView);
		measureHeader(itemView);
